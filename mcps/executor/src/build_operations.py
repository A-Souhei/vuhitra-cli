...
class BuildOperationsTools:
    ...
    def install_npm_packages(self, path: str, packages: Optional[List[str]] = None, package_json: bool = True, dev: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # Validate input: either package_json or packages must be provided
            if not package_json and not packages:
                return {'success': False, 'error': 'Either package_json=True or packages list must be provided'}

            if package_json:
                command = ['npm', 'install']
            else:
                # packages is guaranteed to be not None due to validation above
    ...
    def run_build_command(self, path: str, build_command: str, timeout: int = 300) -> Dict[str, Any]:
        ...
        Security Note:
            ...
            build_command contains unsanitized user input. Ensure commands are 
            from trusted sources only. This is acceptable in a controlled 
            development environment where commands are generated by the LLM
            for mirror+vanisher directories only.
        """
        try:
            resolved_path = self.manager.resolve_path(path)
            if not resolved_path:
            ...
            # WARNING: shell=True is used for build command flexibility
